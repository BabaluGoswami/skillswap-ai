...
import { 
  ...
  Paperclip, FileText, Download, Eye, X 
} from 'lucide-react';
...
const Chat = () => {
  ...
  // Disable html/body scrollbars on mount to prevent any footer layout shifting
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (msg) => {
      if (activeConv && msg.conversationId === activeConv.conversationId) {
        const normalized = normalizeMessage(msg, currentUser?._id);
        setMessages(prev => {
  ...
  const loadMessages = async (convId, before = null) => {
    ...
    if (res.success) {
      const normalized = (res.messages || []).map(m => normalizeMessage(m, currentUser?._id));
      if (before) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 theme-transition h-[calc(100vh-10rem)] flex gap-6 overflow-hidden min-h-0">
      
      {/* Sidebar: Conversation listing */}
      <div className="w-80 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 rounded-3xl shadow-sm flex flex-col overflow-hidden theme-transition shrink-0 hidden md:flex min-h-0">
      ...
      {/* Main chat window */}
      <div className="flex-grow bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 rounded-3xl shadow-sm flex flex-col overflow-hidden theme-transition relative min-h-0">
        {activeConv ? (
          <>
            {/* Header Peer Info */}
